import { handler } from "../../../lambda/blockchainWallets/updateBlockchainWalletFunction"
...
jest.mock("/opt/nodejs/blockchainWalletsLayer", () => ({
    BlockchainWalletRepository: jest.fn().mockImplementation(() => {
        return {
            updateBlockchainWallet: jest.fn(()=> resultSucess),
        };
    }),
}));
...
describe("Update a blockchain wallet data", () => {
    ...
    it("Should successfully update a blockchain wallet", async () => {
        blockchainWalletRepositoryMock.updateBlockchainWallet.mockResolvedValue({
            id: "12345",
            walletName: "test_wallet",
            walletAddress: "test_address",
            privateKey: "test_private_key",
